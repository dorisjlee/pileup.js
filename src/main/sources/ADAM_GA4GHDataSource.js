...
import ADAMAlignment from '../ADAMAlignment';
...
function create(spec: GA4GHSpec): AlignmentDataSource {
  ...
  function addReadsFromResponse(response: Object) {
    response.alignments.forEach(alignment => {
      // optimization: don't bother constructing a ADAMAlignment unless it's new.
      var key = ADAMAlignment.keyFromGA4GHResponse(alignment);
      if (key in reads) return;

      var ga4ghAlignment = new ADAMAlignment(alignment);
